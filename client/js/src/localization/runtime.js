...
export function resolveLocale(value) {
  ...
  return (
    Object.keys(languages).find((locale) => locale === tag.split("-")[0]) ??
    "en"
  );
}
...
export function createTranslator(catalogs, initialLocale = "en") {
  ...
  function prepare() {
    ...
    patterns = Object.keys(catalogs[locale] ?? {})
      .filter(
        (source) =>
          /\{p\d+\}/.test(source) &&
          source.replace(/\{p\d+\}/g, "").trim().length >= 1,
      )
      .sort(
        (a, b) =>
  ...
  function translate(value, depth = 0) {
    ...
    let translated = Object.prototype.hasOwnProperty.call(catalog, text)
      ? catalog[text]
      : undefined;
    ...
    return (
      value.slice(0, value.indexOf(value.trimStart())) +
      translated +
  ...
  return {
    ...
    setNames(values) {
      const next = new Set(
        values.filter((value) => typeof value === "string").map(normalize),
      );
      if (
        next.size === names.size &&
        [...next].every((name) => names.has(name))
      ) {
        return false;
      }
      names = next;
