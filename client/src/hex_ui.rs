...
pub(crate) fn center(pos: Position) -> Vec2 {
    let c = pos.coordinate();
    let p = c.to_pixel(SPACING);
    to_screen(Vec2::new(p.0, p.1))
}

pub(crate) enum HexFeature {
    Exhausted,
    ExplorerToken,
}

pub(crate) fn draw_hex(
    p: Position,
    text_color: Color,
    overlay: Color,
...
pub(crate) fn pixel_to_coordinate(p: Vec2) -> Coordinate {
    let p = to_game(Vec2::new(p.x, p.y));
    Coordinate::from_pixel(p.x, p.y, SPACING)
}

pub(crate) fn rotate_around(center: Vec2, radius: f32, angle_deg: usize) -> Vec2 {
    rotate_around_rad(center, radius, PI / 180.0 * (angle_deg as f32))
}

pub(crate) fn rotate_around_rad(center: Vec2, radius: f32, angle_rad: f32) -> Vec2 {
    Vec2::new(
...
pub(crate) fn to_screen(p: Vec2) -> Vec2 {
    let x = p.x + LEFT_BORDER;
    let y = TOP_BORDER - p.y;
    Vec2::new(x, y)
}

pub(crate) fn to_game(p: Vec2) -> Vec2 {
    let x = p.x - LEFT_BORDER;
    let y = TOP_BORDER - p.y;
    Vec2::new(x, y)
}
