...
fn format_playing_action_log_item(action: &PlayingAction, game: &Game) -> String {
    ...
    match action {
        ...
        PlayingAction::IncreaseHappiness { happiness_increases } => {
            ...
            format!("{player_name} increased happiness in {}", utils::format_collection(&happiness_increases, "no city"))
        },
    ...
    match action.phase {
        crate::status_phase::StatusPhaseState::CompleteObjectives => {
            ...
            format!(
                ...
                utils::format_collection(&completed_objectives, "no objectives")
            )
        }
