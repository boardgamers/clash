...
fn format_playing_action_log_item(action: &PlayingAction, game: &Game) -> String {
    ...
    match action {
        ...
        PlayingAction::Recruit { units, city_position, payment, leader_index, replaced_units } => format!("{player_name} payed {payment} to recruit {}{} in the city at {city_position}{}", units.iter().cloned().collect::<Units>(), leader_index.map_or(String::new(), |leader_index| format!(" {} {} as his leader", if player.available_leaders.len() > 1 { "choosing" } else { "getting" }, &player.available_leaders[leader_index].name)), format_args!("{}{}", match replaced_units.len() { 0 => "", 1 => " replacing the unit at ", _ => " replacing the units at " }, utils::format_list(&replaced_units.iter().map(|unit_id| player.get_unit(*unit_id).expect("the player should have the replaced units").position.to_string()).collect(), ""))),
