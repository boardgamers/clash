...
#[test]
fn test_movement() {
    JSON.test("movement", vec![TestAction::undoable(
        0,
...
fn test_explore_choose() {
    JSON.test("explore_choose", vec![TestAction::not_undoable(
        1,
...
fn test_explore_auto_no_walk_on_water() {
    JSON.test("explore_auto_no_walk_on_water", vec![
        TestAction::not_undoable(0, move_action(vec![0], Position::from_offset("B2"))),
    ]);
}

#[test]
fn test_explore_auto_adjacent_water() {
    JSON.test("explore_auto_adjacent_water", vec![
        TestAction::not_undoable(0, move_action(vec![0], Position::from_offset("C7"))),
    ]);
}

#[test]
fn test_explore_auto_water_outside() {
    JSON.test("explore_auto_water_outside", vec![
        TestAction::not_undoable(1, move_action(vec![1], Position::from_offset("F5"))),
    ]);
}

#[test]
fn test_explore_resolution() {
    JSON.test("explore_resolution", vec![
...
fn test_ship_transport() {
    JSON.test("ship_transport", vec![TestAction::undoable(
        0,
...
fn test_ship_transport_same_sea() {
    JSON.test("ship_transport_same_sea", vec![TestAction::undoable(
        0,
...
#[test]
fn test_ship_embark() {
    JSON.test("ship_embark", vec![TestAction::undoable(
        0,
...
fn test_ship_embark_continue() {
    JSON.test("ship_embark_continue", vec![TestAction::undoable(
        0,
        Action::Movement(Move(MoveUnits::new(
            vec![5, 6],
            Position::from_offset("C3"),
...
fn test_ship_disembark() {
    JSON.test("ship_disembark", vec![TestAction::undoable(
        0,
...
fn test_ship_disembark_capture_empty_city() {
    JSON.test("ship_disembark_capture_empty_city", vec![
        TestAction::undoable(0, move_action(vec![1, 2], Position::from_offset("B2"))).skip_json(),
        TestAction::undoable(
            0,
...
fn test_ship_explore() {
    JSON.test("ship_explore", vec![TestAction::not_undoable(
        1,
...
fn test_ship_explore_teleport() {
    JSON.test("ship_explore_teleport", vec![TestAction::not_undoable(
        1,
...
fn test_ship_explore_move_not_possible() {
    JSON.test("ship_explore_move_not_possible", vec![
        TestAction::undoable(1, Action::Response(EventResponse::ExploreResolution(3))),
    ]);
}
...
fn test_ship_navigate() {
    JSON.test("ship_navigate", vec![TestAction::undoable(
        1,
...
fn assert_navigate(game: &mut Game, from: Position, to: Position) {
    ...
    let result = possible_move_units_destinations(game.player(1), game, &[1], from, None)
...
fn test_ship_navigate_explore_move() {
    JSON.test("ship_navigate_explore_move", vec![
        TestAction::not_undoable(1, move_action(vec![1], Position::from_offset("F2"))),
    ]);
}

#[test]
fn test_ship_navigate_explore_not_move() {
    JSON.test("ship_navigate_explore_not_move", vec![
        TestAction::not_undoable(1, move_action(vec![1], Position::from_offset("F2"))),
    ]);
}
