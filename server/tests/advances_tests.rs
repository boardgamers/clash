...
use server::content::custom_actions::CustomAction::{
    AbsolutePower, ArtsInfluenceCultureAttempt, Bartering, CivilLiberties, ForcedLabor, Sports,
...
fn test_bartering() {
    JSON.test(
        "bartering",
        vec![
            TestAction::undoable(0, Action::Playing(Custom(Bartering))).without_json_comparison(),
            TestAction::undoable(
                0,
