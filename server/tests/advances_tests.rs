...
use server::movement::{move_units_destinations, MoveUnits};
    ...
    JSON.test("forced_labor", vec![
        ...
        TestAction::undoable(
            ...
            Action::Playing(Collect(playing_actions::Collect::new(
                ...
                ResourcePile::food(1) + ResourcePile::wood(1),PlayingActionType::Collect,
            ))),
        ),
    ]);
}
...
fn test_collect_husbandry() {
    let action = Action::Playing(Collect(playing_actions::Collect::new(
        ...
        ResourcePile::food(1),                              PlayingActionType::Collect,
    )));
