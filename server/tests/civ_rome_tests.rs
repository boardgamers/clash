...
#[test]
fn provinces() {
    JSON.test("provinces", vec![
        TestAction::undoable(0, advance_action(Advance::Voting, ResourcePile::food(2))).skip_json(),
...
#[test]
fn imperator() {
    JSON.test("imperator", vec![TestAction::not_undoable(
        0,
...
#[test]
fn statesman() {
    JSON.test("statesman", vec![TestAction::undoable(
        0,
...
#[test]
fn proconsul() {
    JSON.test("proconsul", vec![
        TestAction::not_undoable(0, move_action(vec![0], Position::from_offset("D8"))).skip_json(),
        TestAction::undoable(0, payment_response(ResourcePile::gold(1))),
    ]);
}
