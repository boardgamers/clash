...
for (const locale of locales) {
  const catalog = JSON.parse(
    await readFile(join(directory, `${locale}.json`), "utf8"),
  );
  assert.deepEqual(
    Object.keys(catalog).sort(),
    Object.keys(source).sort(),
    `${locale}: missing or extra strings`,
  );
  for (const [key, text] of Object.entries(catalog)) {
    ...
    assert.deepEqual(
      ...
      `${locale}: changed placeholders in ${key}`,
    );
  }
}
console.log(
  `${locales.length} languages × ${Object.keys(source).length} messages validated`,
);
